...
class ActivationWizardController extends BaseController {
	...
	public function save_activation_settings( $data ) {
		...
		if ( array_key_exists( 'base_color', $data ) ) {
			$color = sanitize_hex_color( $data['base_color'] );
			if ( $color !== '' && $color !== null) {
		...
		if ( array_key_exists( 'agent_color', $data ) ) {
			$color = sanitize_hex_color( $data['agent_color'] );
			if ( $color !== '' && $color !== null) {
		...
		if ( array_key_exists( 'client_color', $data ) ) {
			$color = sanitize_hex_color( $data['client_color'] );
			if ( $color !== '' && $color !== null) {
		...
		if ( array_key_exists( 'buttons_color', $data ) ) {
			$color = sanitize_hex_color( $data['buttons_color'] );
			if ( $color !== '' && $color !== null) {
