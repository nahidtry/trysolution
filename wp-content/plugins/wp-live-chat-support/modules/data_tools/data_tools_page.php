...
function wplc_export_ratings() {
	...
	$csvdata = $wpdb->get_results(
		"
        SELECT $wplc_tblname_chats.id as 'Chat ID', $wplc_tblname_chats.timestamp as 'Time', $wplc_tblname_chats.agent_id as 'Agent ID',users.display_name as 'agent_name', case when $wplc_tblname_chat_ratings.rating=1 then 'Satisfied' when $wplc_tblname_chat_ratings.rating=0 then 'Unsatisfied' else 'Not rated' end  as 'Rating'
